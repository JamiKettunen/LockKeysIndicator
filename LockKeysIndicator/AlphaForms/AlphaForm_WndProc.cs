...
namespace LockKeysIndicator // AlphaForms by Anthony Mushrow
{
    public partial class AlphaForm
    {
        ...
        private bool dblClick(Point pos)
        {
            ...
            Size dist = new Size
            {
                Width = Math.Abs(m_lockedPoint.X - pos.X),
                Height = Math.Abs(m_lockedPoint.Y - pos.Y)
            };
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            switch (msgId)
            {
                ...
                case Win32.Message.WM_MOUSEMOVE:
                    ...
                    if (SafeNativeMethods.GetCapture() != IntPtr.Zero && m_moving)
                    {
                        ...
                        if (m_enhanced)
                        {
                            ...
                            IntPtr hdcScreen = SafeNativeMethods.GetWindowDC(m_layeredWnd.Handle);  //Screen DC that the layered window will draw to
                            IntPtr windowDC = SafeNativeMethods.GetDC(this.Handle);                 //Window DC that we are going to copy
                            IntPtr memDC = SafeNativeMethods.CreateCompatibleDC(windowDC);          //Temporary DC that we draw to
                            IntPtr BmpMask = SafeNativeMethods.CreateBitmap(this.ClientSize.Width,
                            ...
                            //Create mask
                            SafeNativeMethods.SelectObject(memDC, BmpMask);
                            uint oldCol = SafeNativeMethods.SetBkColor(windowDC, 0x00FF00FF);
                            SafeNativeMethods.BitBlt(memDC, 0, 0, this.ClientSize.Width, this.ClientSize.Height, windowDC, 0, 0, Win32.TernaryRasterOperations.SRCCOPY);
                            SafeNativeMethods.SetBkColor(windowDC, oldCol);
                            ...
                            SafeNativeMethods.SelectObject(memDC, BmpBack);
                            IntPtr brush = SafeNativeMethods.CreateSolidBrush(0x00FFFFFF);
                            SafeNativeMethods.SelectObject(memDC, brush);
                            SafeNativeMethods.MaskBlt(memDC, 0, 0, backImage.Width, backImage.Height, windowDC, 0, 0, BmpMask, 0, 0, 0xCFAA0020);
                            ...
                            {
                                AlphaFormat = (byte)Win32.BlendOps.AC_SRC_ALPHA,
                                BlendFlags = (byte)Win32.BlendFlags.None,
                                BlendOp = (byte)Win32.BlendOps.AC_SRC_OVER,
                                SourceConstantAlpha = (byte)(this.Opacity * 255)
                            };

                            SafeNativeMethods.UpdateLayeredWindow(m_layeredWnd.Handle, hdcScreen, ref pos, ref size, memDC, ref zero, 0, ref blend, Win32.BlendFlags.ULW_ALPHA);

                            //Clean up
                            SafeNativeMethods.ReleaseDC(IntPtr.Zero, hdcScreen);
                            SafeNativeMethods.ReleaseDC(this.Handle, windowDC);
                            SafeNativeMethods.DeleteDC(memDC);
                            SafeNativeMethods.DeleteObject(brush);
                            SafeNativeMethods.DeleteObject(BmpMask);
                            SafeNativeMethods.DeleteObject(BmpBack);
                        ...
                        SafeNativeMethods.ReleaseCapture();
                        SafeNativeMethods.SendMessage(this.Handle, (int)Win32.Message.WM_NCLBUTTONDOWN, (int)Win32.Message.HTCAPTION, 0);
                ...
                case Win32.Message.WM_WINDOWPOSCHANGING:
                    {
                        ...
                        if ((posInfo.flags & move_size) != move_size)
                        {
                            ...
                            {
                                IntPtr hwdp = SafeNativeMethods.BeginDeferWindowPos(2);
                                if (hwdp != IntPtr.Zero)
                                    hwdp = SafeNativeMethods.DeferWindowPos(hwdp, m_layeredWnd.Handle, this.Handle, posInfo.x + m_offX, posInfo.y + m_offY,
                                            0, 0, (uint)(posInfo.flags | Win32.WindowPosFlags.SWP_NOSIZE | Win32.WindowPosFlags.SWP_NOZORDER));
                                if (hwdp != IntPtr.Zero)
                                    hwdp = SafeNativeMethods.DeferWindowPos(hwdp, this.Handle, this.Handle, posInfo.x, posInfo.y, posInfo.cx, posInfo.cy,
                                            (uint)(posInfo.flags | Win32.WindowPosFlags.SWP_NOZORDER));
                                if (hwdp != IntPtr.Zero)
                                    SafeNativeMethods.EndDeferWindowPos(hwdp);
                ...
                case Win32.Message.WM_ACTIVATE:
                    {
                        ...
                        if (m.WParam != IntPtr.Zero)
                        {
                            ...
                            IntPtr prevWnd = SafeNativeMethods.GetWindow(m_layeredWnd.Handle, Win32.GetWindow_Cmd.GW_HWNDPREV);
                            while (prevWnd != IntPtr.Zero)
                            {
                                //If we find a visiable window, we stop
                                if (SafeNativeMethods.IsWindowVisible(prevWnd))
                                    break;

                                prevWnd = SafeNativeMethods.GetWindow(prevWnd, Win32.GetWindow_Cmd.GW_HWNDPREV);
                            }

                            //If the visible window isn't ours reset the position of the background form
                            if (prevWnd != this.Handle)
                                SafeNativeMethods.SetWindowPos(m_layeredWnd.Handle, this.Handle, 0, 0, 0, 0, (uint)(Win32.WindowPosFlags.SWP_NOSENDCHANGING | Win32.WindowPosFlags.SWP_NOACTIVATE | Win32.WindowPosFlags.SWP_NOSIZE | Win32.WindowPosFlags.SWP_NOMOVE));
                        }
                    }
                    break;
            }
